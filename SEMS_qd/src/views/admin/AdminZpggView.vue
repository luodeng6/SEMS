          ...
          <el-table :data="paginatedData" border style="width: 100%">
            ...
            <el-table-column label="操作" width="200">
              <template #default="scope">
                ...
                <el-button
                    :type="scope.row.QYDM ? 'danger' : 'success'"
                    size="mini"
                    @click="toggleStatus(scope.row)"
                >
                  {{ scope.row.QYDM ? '停用' : '启用' }}
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-tab-pane label="发布公告" name="publish">
          ...
          <el-form ref="formRef" :model="form" :rules="rules" class="form-container">
            ...
            <el-form-item label-width="80px" prop="GGNR">
              <div id="editor-container" style="border: 1px solid #ccc; min-height: 600px"></div>
            </el-form-item>
          </el-form>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  methods: {
    ...
    // 加载公告列表
    getGongGaoList() {
      axios.get(`/dwzpggk/getdwzpggk?YFSFDM=1&YHM=${this.UserInfo.username}&QYDM=-1`).then(res => {
        if (res.data.result) {
    ...
    handleEdit(notice) {
      ...
      this.editor.txt.html(notice.GGNRHTML);
