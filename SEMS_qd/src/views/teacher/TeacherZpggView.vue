        ...
        <el-tab-pane label="公告列表" name="list">
          <el-table   @row-dblclick="handleRowDblClick" :data="paginatedData" v-loading="isLoading"  element-loading-text="加载中..."
            ...
            <el-table-column label="操作" width="200">
              <template #default="scope">
                <el-button size="mini"  v-show="scope.row.FBZ===UserInfo.username"  @click="handleEdit(scope.row)">编辑</el-button>
                <el-button
                    :type="scope.row.QYDM===1 ? 'danger' : 'success'"
                    size="mini"
                    v-show="scope.row.FBZ===UserInfo.username"
                    @click="toggleStatus(scope.row)"
                >
                  {{ scope.row.QYDM===1 ? '停用' : '启用' }}
                </el-button>
                <span style="color: red" v-if="scope.row.FBZ!==UserInfo.username">无权操作</span>
              </template>
                ...
                <div>
                  ...
                  <el-form :model="form" label-width="80px">
                    ...
                    <el-form-item>
                      ...
                      <el-button type="success" @click="handlePublish">立即发布</el-button>
                    </el-form-item>
                  </el-form>
                </div>
              </el-card>
...
export default {
  ...
  methods: {
    handleRowDblClick(row){
      console.log(row);
      this.$router.push({path: '/teacher/TeacherZpggDetailView', query: {id: row.ID}})
    },
    formatDate(dateStr) {
    ...
    // 加载公告列表
    getGongGaoList() {
      axios.get(`/dwzpggk/getdwzpggk?YFSFDM=2&YHM=${this.UserInfo.username}&QYDM=-1`).then(res => {
        if (res.data.result) {
      ...
      this.$refs.formRef.validate(valid => {
        if (valid) {
          if (this.isEditMode) {
            ...
            dataForm.append('ggnr', this.form.GGNR);
            dataForm.append('fbz', this.UserInfo.username);
            dataForm.append("fbzsfdm",2);//单位用户
            dataForm.append('ggnrhtml', this.form.GGNRHTML);
