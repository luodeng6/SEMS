    ...
    <main class="ml-64 flex-1 p-6 bg-gray-50 min-h-screen">
      ...
      <el-card class="!border-none  " shadow="never">
        ...
        <div class="border-b pb-4 mb-6">
          <h1 class="text-3xl font-bold text-gray-800 mb-2">{{ detail.GSMC }}-{{ detail.DWMC }} -招聘简章</h1>
          <div class="flex items-center text-sm text-gray-500 space-x-4">
            <span>创建者：{{ detail.CJZMC }}</span>
            <span>创建时间：{{ formatTime(detail.CJSJ) }}</span>
        ...
        <div class="mt-8 pt-4 border-t text-sm text-gray-500">
          <p>发布者身份代码：{{ detail.GSMC }}</p>
          <p>启用代码：{{ detail.FBBZ }}</p>
        </div>
      </el-card>
    </main>
  </div>
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.id) {
      this.ZPJZID = this.$route.query.id;
